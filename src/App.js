...
import d3-selection from 'd3-selection';
import d3-collection from 'd3-collection';
import d3-time-format from 'd3-time-format';
...
class App extends Component {
  ...
  scaffold = (results, graphDiv) => () => {
      ...
      var svg = d3-selection.select(graphDiv).append('svg').attr('height', $graphDiv.height()).attr('width', $graphDiv.width());
      canvas.node = svg.append('g').attr('transform', 'translate(' + canvas.margin.left + ',' + canvas.margin.top + ')');

      canvas.dataset = d3-collection.nest()
        .key(function (d) {
          return d.name
        })
        .rollup(function (d) {
  ...
  populateGraph = () => {
    var timeParse = d3-time-format.timeParse('%Y-%m-%dT%H:%M');
    var formatTime = d3-time-format.timeFormat('%Y-%m-%dT%H:%M');
    ...
    var tempVariance = d3-collection.nest()
      .key(function (d) {
  ...
  render() {
    if (this.state.weatherData && this.state.weatherData.apiResults.results.length > 0)
        var t = this.scaffold(this.state.weatherData,'.graph-canvas');
